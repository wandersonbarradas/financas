...
export const ModalTransaction = (props: Props) => {
    ...
    useEffect(() => {
        if (props.type === "expense") {
            setColorTransaction({
                solid: state.theme.theme.expenseColor,
                rgba: "rgba(240, 41, 39, 0.38)",
            });
        } else if (props.type === "income") {
            setColorTransaction({
                solid: state.theme.theme.incomeColor,
                rgba: "rgba(79, 209, 139, 0.38)",
            });
        } else {
            setColorTransaction({
                solid: state.theme.theme.transferColor,
                rgba: "rgba(0, 141, 255, 0.38)",
            });
        }
        getCategory();
    }, []);
    ...
    useEffect(() => {
        ...
        if (props.item.type !== "transfer") {
            ...
            const d = props.item.date as {
                seconds: number;
                nanoseconds: number;
            };
            setDateTransaction(new Date(d.seconds * 1000));
        } else if (props.item.type === "transfer") {
            const t = props.item as TransferTansactionType;
            setValueTransaction(t.value);
            setDescription(t.description);
            setAccount(t.account);
            setAccountFor(t.accountFor);
            const d = t.date as {
                seconds: number;
                nanoseconds: number;
            };
    ...
    const getCategory = async () => {
        ...
        const resultCat = (await Api.getUserDocument(
            state.user.data.id,
            "categories",
        )) as CategoryType[];
        ...
        const resultSubcat = (await Api.getUserDocument(
            state.user.data.id,
            "subcategories",
        )) as SubCategories[];
    ...
    const createTransaction = async () => {
        ...
        if (props.type === "transfer") {
            ...
            const transactionsArr = state.user
                .transactions as TransferTansactionType[];
            const newTransactionsList = transactionsArr.filter(
                (item) => item.id !== id,
            );
            transaction.date = {
    ...
    const handleBtnDate = (e: React.MouseEvent<HTMLElement>) => {
        ...
        if (element.innerText === "Hoje") {
            ...
            setDateTransaction(
                new Date(ontem.year(), ontem.month(), ontem.date()),
            );
        } else {
    ...
    const backgroundBtnDate = (btnIndex: number) => {
        let items: NodeListOf<HTMLAnchorElement> =
            document.querySelectorAll(".btn-date");
        if (items.length <= 0) {
            return false;
        }
    ...
    return (
        ...
        >
            <Modal
                ...
                setOpen={setModalDatePiker}
                modalOpacity={0.5}
                clickAway={true}
            >
                <Calendario
                    value={dateTransaction}
                    dateValue={handleDateExpense}
                    handleModal={handleModalDatePicker}
                />
            </Modal>
            ...
            <div className="bodyContent">
                <div className="left-side">
                    <div className="input-area date" onClick={handleBtnDate}>
                        ...
                        <button
                            className="btn-date"
                            onClick={() => handleModalDatePicker("open")}
                        >
                            Outro...
                        </button>
                        <span className="date-extense">
                            {DF.getDateExtense(dateTransaction)}
                        </span>
                    </div>
                    <label htmlFor="input-value-expense">
                        <div
                            className="input-area value"
                        ...
                        >
                            ...
                            <span>R$</span>
                            <input
                                onBlur={() => handleVerificDescription("value")}
                                type="number"
                                placeholder="0,00"
                                id="input-value-expense"
                                onChange={handleValue}
                                value={
                                    valueTransaction > 0 ? valueTransaction : ""
                                }
                            />
                        </div>
                    </label>
                    ...
                    <label htmlFor="input-description-expense">
                        ...
                        >
                            ...
                            <input
                                onBlur={() =>
                                    handleVerificDescription("description")
                                }
                                type="text"
                    ...
                    {props.type !== "transfer" && (
                        <label
                            htmlFor="input-category-expense"
                            onClick={() => setModalCategories(true)}
                        >
                            <div
                                className="input-area category"
                                onClick={handleFocusInputArea}
                            >
                                <div className="icon">
                    ...
                    <label
                        htmlFor="input-account-expense"
                        onClick={() => setModalAccount(true)}
                    >
                        <div
                            className={
                                props.type === "transfer"
                                    ? "input-area account tr"
                                    : "input-area account"
                            }
                            onClick={handleFocusInputArea}
                        >
                            ...
                            >
                                ...
                                {!account && (
                                    <input
                                        onBlur={() =>
                                            handleVerificDescription("account")
                                        }
                    ...
                    {props.type === "transfer" && (
                        <label
                            htmlFor="input-account-for-expense"
                            onClick={() => setModalAccountFor(true)}
                        >
                            <div
                                className="input-area accountFor"
                                onClick={handleFocusInputArea}
                            >
                                <div className="icon">
                                    <AccountBalanceOutlinedIcon />
            ...
            <div className="footerModalTransaction">
                {props.type === "transfer" && (
                    <button onClick={createTransaction} disabled={disabledBtn}>
                        Salvar
                    </button>
                )}
                {props.type !== "transfer" && (
            ...
            <Modal
                clickAway={true}
                modalOpacity={0.5}
                open={modalAccountFor}
                setOpen={setModalAccountFor}
            >
                <div className="containerAccounts">
                    {accounts.map((item, index) => (
                        <C.AccountItem
                            key={index}
                            onClick={() => handleAccountForItem(item)}
                            Theme={state.theme.theme}
                        >
                            <img src={item.account.imgUrl} alt="" />
                            <span>{item.description}</span>
